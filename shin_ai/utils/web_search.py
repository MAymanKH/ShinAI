    ...
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        for item in results:
            ...
            final_results.append(
                {"title": title, "url": item.get("url") or "", "snippet": snippet, "content": content}
            )
...
async def search_web_tool(query: str) -> str:
    ...
    logger.info(
        f"Executing web search tool for query: '{query}' (Request count: {current_count}, remaining time: {remaining_time:.1f}s)"
    )
    ...
    if firecrawl_key:
        ...
        if rem_time > 0:
            try:
                logger.info(
                    f"Attempting Firecrawl search for query: '{query}' (remaining time: {rem_time:.1f}s)"
                )
                output_json = await asyncio.wait_for(
                    _do_firecrawl_search(query, firecrawl_key, rem_time), timeout=rem_time
                )
    ...
    async def _do_search(deadline: float):
        ...
        async with httpx.AsyncClient(headers=headers) as client:
            ...
            for index, res in enumerate(results_list):
                content = (
                    contents[index]
                    if index < len(contents) and not isinstance(contents[index], Exception)
                    else ""
                )
                final_results.append(
                    {
    ...
    try:
        ...
        if "ddgsexception" in type(e).__name__.lower() or "no results found" in err_msg.lower():
            ...
            return _format_error_as_result(
                query, f"DuckDuckGo search failed or returned no results: {err_msg}"
            )
